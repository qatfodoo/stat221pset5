...
## Slurm job to generate local output for router 2

## question 1.9 on Odyssey
...
if (Sys.getenv("SLURM_JOB_ID") != "") { # Divide computation per tasks
  ...
  win <- replicate(N.t, matrix(data=0, nrow=15, ncol=w), simplify=F)
  for (t in 1:N.t) {
    win[[t]] <- y[, t:(t + w - 1)]
  }
  ...
  for (i in 1:length(win.id)) {
    ...
    y.t <- win[[t]]
    theta.part[[i]]$t <- t
    A <- RoutingMatrix(I.par)
    EM <- iid_EM(y.t, m.step=m.step, I.par=I.par, A=A)
    theta.part[[i]]$theta <- EM$theta
    theta.part[[i]]$q.list <- EM$q.list
  }
  
  save(theta.part, file=paste("./out/2router_theta_part_", task.id, ".dat", sep=""))
